defmodule XMLConverter do
  @moduledoc """
  In order to get quick lookups for the words in the dictionary files, we need to convert original
  XML dictionary files into SQLite counterparts with a simple structure.
  ...
  defp insert({id, variants, word, definition}, {conn, total, processed}) do
    Enum.each(variants, fn {word, type} ->
      ...
      {:ok, statement} = SQLite.prepare(conn, word_sql)
      :ok = SQLite.bind(conn, statement, [id, word, type])
      :done = SQLite.step(conn, statement)
    end)

    definition_sql = "INSERT INTO definitions (id, word, definition) VALUES (?1, ?2, ?3)"

    {:ok, statement} = SQLite.prepare(conn, definition_sql)
    :ok = SQLite.bind(conn, statement, [id, word, definition])
    :done = SQLite.step(conn, statement)
  ...
  def main(args \\ []) do
    ...
    case options do
      [input: input_filename, output: output_filename] ->
        XMLConverter.convert(input_filename, output_filename)
...
System.argv() |> dbg() |> XMLConverterRunner.main()
